...
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t lv_color_mix(lv_color_t c1, lv_color_t c2, uint8_t mix)
{
    ...
    if(LV_COLOR_GET_A(c2) < LV_OPA_MIN && LV_COLOR_GET_A(c1) > LV_OPA_MAX)
    {
        LV_COLOR_SET_A(ret, mix);
    }
    else if ( LV_COLOR_GET_A(c2) < LV_OPA_MAX && mix < LV_OPA_MAX){
        uint16_t fa,a;
        fa = (uint16_t)LV_COLOR_GET_A(c2);
        a = 255 - (((255 - fa) * (255 - mix)) >> 8);
        LV_COLOR_SET_A(ret, a);
    }
    else
#endif
    {
...
#if LV_COLOR_DEPTH == 32
    if(LV_COLOR_GET_A(c2) < LV_OPA_MIN)
    {
        LV_COLOR_SET_A(ret, mix);
    }
    else if ( LV_COLOR_GET_A(c2) < LV_OPA_MAX && mix < LV_OPA_MAX){

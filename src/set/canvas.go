package set

import types "lvgl-go/src/types"

/*
#cgo CFLAGS: -I../include/
#cgo LDFLAGS: -L../lib -llvgl
#include "lv_init.h"
*/
import "C"
import (
	"unsafe"
)

type SetCanvas set

func CreateCanvas(o *types.LvObjT) SetCanvas {
	return SetCanvas{
		CStructLvObjT: (*C.struct__lv_obj_t)(unsafe.Pointer(o)),
	}
}

func (setter SetCanvas) GetObj() *types.LvObjT {
	return (*types.LvObjT)(unsafe.Pointer(setter.CStructLvObjT))
}

func (setter SetCanvas) SetBuffer(buf any, w types.LvCoordT, h types.LvCoordT, cf types.LvImgCfT) SetCanvas {
	C.lv_canvas_set_buffer(setter.CStructLvObjT, unsafe.Pointer(&buf), C.lv_coord_t(w), C.lv_coord_t(h), C.lv_img_cf_t(cf))

	return setter
}
func (setter SetCanvas) SetPxColor(x types.LvCoordT, y types.LvCoordT, c types.LvColorT) SetCanvas {
	C.lv_canvas_set_px_color(setter.CStructLvObjT, C.lv_coord_t(x), C.lv_coord_t(y), C.lv_color_t(c))

	return setter
}
func (setter SetCanvas) SetPxOpa(x types.LvCoordT, y types.LvCoordT, opa types.LvOpaT) SetCanvas {
	C.lv_canvas_set_px_opa(setter.CStructLvObjT, C.lv_coord_t(x), C.lv_coord_t(y), C.lv_opa_t(opa))

	return setter
}
func (setter SetCanvas) SetPalette(id uint8, c types.LvColorT) SetCanvas {
	C.lv_canvas_set_palette(setter.CStructLvObjT, C.uint8_t(id), C.lv_color_t(c))

	return setter
}
